...
def data_handle():
    ...
    # 创建交易日基类实例
    trade_date_base = TradeDateBase(start_date)
    # 获取查询起始日到今天全部交易日列表
    trade_date_list_to_today = trade_date_base.get_trade_date_list_to_today()

    # 创建龙虎榜数据接口基类实例
    # 遍历交易日列表获取龙虎榜符合条件股票代码列表
    df_list = []
    for trade_date in trade_date_list_to_today:
        print(trade_date)
        try:
            trade_date_zone = TradeDateBase(trade_date)
            trade_date_zone.day_num = 5
            # 获取自定义区间内交易日列表
            trade_date_list = trade_date_zone.get_trade_date_list_by_num()
        ...
        for code in code_list:
            try:
                final_df = get_stock_price_in_n_days(code, trade_date, trade_date_list)
    ...
    # TODO: 优化 excel_utility，兼容 Windows 与 linux
    excel_utility.append_to_new_sheet(all_data_pd, './data/dragon_tiger_data_output.xlsx',
...
def get_stock_price_in_n_days(code, query_date, trade_date_list):
    ...
    df_price = ts.pro_bar(ts_code=code, adj='qfq', start_date=start_date, end_date=end_date, freq='D')
    if df_price is None:
        print(code + "：无行情数据")
